...
if (!empty($email) && !empty($apiKey)) {
    if ($conn) {
        ...
        if (mysqli_num_rows($res) != 0) {
            ...
            $result = array("status" => "success", "message" => "Data Fetched Successfully", "name" => $row['userName'], "email" => $row['userEmail'], "contact" => $row['userContact'], "address" => $row['userAddress'], "nic" => $row['userNIC']);
        } else {
